...
async function checkTotalSize(req, res, next) {
    try{
        ...
        if(size>=MAX_SIZE){
            throw specialError('Upload directory full. Cannot upload until space is freed.',403);
        } else if (size+reqSize>=MAX_SIZE){
            throw specialError(`Processing request exceeds upload directory size limit. Can upload ${(MAX_SIZE-size)/(1024*1024)}MB only.`,403);
        } else if (reqSize > MAX_SIZE) {
            throw specialError(`Request size exceeds ${(MAX_SIZE/(1024*1024*1024))}GB.`,403);
        } else if (num>MAX_NUM){
            throw specialError('Upload directory has too many files. Cannot upload until number of files reduced.',402);
        } else if (num+reqNum>MAX_NUM){
            throw specialError(`Processing request exceeds number of files limit. Can upload ${MAX_NUM-num} files only.`,402);
        }else{
        next();
        }
    } catch(err) {
